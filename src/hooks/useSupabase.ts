  ...
  return useQuery({
    ...
    queryFn: async () => {
      if (!supabase || !userId) {
        ...
        const saved = JSON.parse(localStorage.getItem('itone-saved-songs') || '[]');
        return saved.filter((s: any) => s.user_id === userId);
      }
...
export function useSaveSong() {
  ...
  return useMutation({
    mutationFn: async ({ userId, songId, transposeValue = 0, notes = '' }: {
      ...
      if (!supabase) {
        // Mock save for demo - Store in localStorage for persistence
        const saved = JSON.parse(localStorage.getItem('itone-saved-songs') || '[]');
        const newSong = { 
          ...
          notes,
          created_at: new Date().toISOString(),
          song: mockSongs.find(s => s.id === songId)
        };
        saved.push(newSong);
        localStorage.setItem('itone-saved-songs', JSON.stringify(saved));
        return newSong;
      }
...
export function useRemoveSong() {
  ...
  return useMutation({
    mutationFn: async ({ userId, songId }: { userId: string; songId: string }) => {
      if (!supabase) {
        // Mock remove for demo - Remove from localStorage
        const saved = JSON.parse(localStorage.getItem('itone-saved-songs') || '[]');
        const filtered = saved.filter((s: any) => !(s.user_id === userId && s.song_id === songId));
        localStorage.setItem('itone-saved-songs', JSON.stringify(filtered));
        return;
      }
